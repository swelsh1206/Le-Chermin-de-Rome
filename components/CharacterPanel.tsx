...
const CharacterPanel: React.FC<CharacterPanelProps> = ({ player, imageUrl, gameState, onOpenWindow }) => {
  ...
  return (
    <div className={`bg-stone-800/80 p-4 border-2 ${borderColor} shadow-lg text-center h-full flex flex-col transition-colors duration-500 rounded-xl`}>
      ...
      <div className={`w-full aspect-square mx-auto border-4 ${borderColor} overflow-hidden mb-4 transition-colors duration-500 rounded-lg`}>
        {imageUrl ? (
          <img src={imageUrl} alt={`Portrait of ${player.name}`} className="w-full h-full object-cover" />
        ) : (
