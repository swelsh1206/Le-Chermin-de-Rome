...
const GameUI: React.FC<GameUIProps> = ({ player, initialGameState, characterImageUrl, onGameEnd }) => {
  ...
  const addLogEntry = useCallback((message: string, color: string = 'text-gray-300') => {
    setGameState(prevGameState => {
        setLog(prevLog => [...prevLog, { day: prevGameState.day, message, color }]);
        ...
        case 'Party': return (
            <ul className="text-lg space-y-3 text-gray-200">
                {gameState.party.map(member => (
                    <li key={member.name} className="border-b border-amber-600/20 pb-2">
                        <div className="flex justify-between items-center mb-1">
                            <span className="font-bold">{member.name}</span>
                            <span className={member.health < 50 ? 'text-red-400' : 'text-green-400'}>Health: {member.health} / 100</span>
                        </div>
                        {member.conditions.length > 0 && (
                            <div className="text-sm mt-1">
                                Conditions: <span className="font-semibold">{member.conditions.join(', ')}</span>
                            </div>
                        )}
                        <div className="text-right mt-2">
                            <button onClick={() => handleOpenInventoryForTarget(member)} className="text-sm px-2 py-1 border border-sky-400 text-sky-400 hover:bg-sky-400 hover:text-stone-900 rounded-md">
                                Give Item
                            </button>
                        </div>
                    </li>
